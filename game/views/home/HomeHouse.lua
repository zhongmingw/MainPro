--
-- Author: wx
-- Date: 2017-11-16 17:37:21
-- 升级住宅

local HomeHouse = class("HomeHouse", base.BaseView)

local _type = 1001
local _level = "houseLev"
local _tilteicon = "jiayuan_040"

function HomeHouse:ctor()
    self.super.ctor(self)
    self.uiLevel = UILevel.level3 
    self.openTween = ViewOpenTween.scale
end

function HomeHouse:initView()
...
function HomeHouse:initData()
    -- body
    self:setData()
end

function HomeHouse:cellData(index, obj)
    -- body
    ...
    --string.format(language.home67,lv)

    obj.data = data
end

function HomeHouse:setData(data_)
    self.money.text = cache.PlayerCache:getTypeMoney(MoneyType.home)
...
function HomeHouse:onClickItemCall(context)
    -- body
    local data = context.data.data
end

function HomeHouse:onHouseUp()
    -- body
