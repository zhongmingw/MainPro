...
local Active1032 = class("Active1032",import("game.base.Ref"))

local totalTime = 24*60*60

function Active1032:ctor(param)
    self.view = param
    self:initView()
end

function Active1032:initView()
    -- body
...
function Active1032:initDec()
    ...
    dec1.text = mgr.TextMgr:getTextByTable(language.kaifu47)
    ...
    mgr.GuiMgr:registerMoneyPanel(self.view,"dayactive.DayActiveView")
end

function Active1032:onTimer()
    -- body
...
function Active1032:setCurId(id)
    -- body
    self.id = id 
end

function Active1032:celldata( index, obj )
    -- body
...
function Active1032:onget(context)
...
function Active1032:onChongzhi()
    -- body
    mgr.ViewMgr:closeAllView2()
    GGoVipTequan(0)
end

function Active1032:add5030116(data)
    -- body
    --*printt(data)
    self.data = data
