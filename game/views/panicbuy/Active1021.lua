...
local Active1021 = class("Active1021",import("game.base.Ref"))
--获取配置
local opent = {
...
--技能最大等级
local maxlv = 9
function Active1021:ctor(mParent,panelObj)
    self.mParent = mParent
    self.panelObj = panelObj
    self:initPanel()
end

function Active1021:initPanel()
    -- body
    ...
    for i = 10,14 do
        ...
        btn.data = i - 9
        btn.title = language.thqg09[btn.data]
...
function Active1021:celldata(index, obj)
    -- body
...
function Active1021:onSelect(context)
    -- body
    local data = context.sender.data
    self:sendMsg(data) 
end

function Active1021:onBtnBuy(context)
    -- body
    if not self.data then
        return
    end
    ...
    proxy.ActivityProxy:sendMsg(1030142,param)
end

function Active1021:onClickCz()
    -- body
    GOpenView({id = 1042})
end

function Active1021:setVisible(flag)
    -- body
...
function Active1021:sendMsg(id)
    -- body
    local param = {}
    param.reqType = 0
    param.itemId = 0
...
function Active1021:findNeed()
    -- body
    local t = {[1001] = 0,[1002] = 3,[1003] = 1,[1004] = 4 , [1005] = 2}
    local condata = conf.ZuoQiConf:getSkillData(t[self.data.moduleId])
    self.need = {}
    for k ,v in pairs(condata) do
        local skillLv = self.data.skillMap[v.id] or 0
        for i = skillLv , maxlv do
            local data = conf.ZuoQiConf:getSkillByLev(v.id,i,t[self.data.moduleId])
...
function Active1021:add5030142( data )
    -- body
    self.data = data
    self.money.text = cache.PlayerCache:getTypeMoney(MoneyType.gold)
    if data.reqType == 0 then
        --获取对应配置
        self.confData = conf.ActivityConf:getHorseSkillSale(data.moduleId)
        --检索需要的道具
        self:findNeed()
        --
        -- --是否可购买
