...
function BubblePanel:initPanel()
    ...
    --基础属性列表
    self.attrsList = self.view:GetChild("n24")
    self.attrsList.numItems = 0

    --升星属性列表
    self.starAttrsList = self.view:GetChild("n25")
    ...
    --当前战力
    self.powerTxt = self.view:GetChild("n28")
    self.powerTxt.text = ""
    --气泡Icon
    self.bubbleIcon = self.view:GetChild("n22")
    --当前选择气泡名称
    ...
    --获取途径
    self.gainTxt = self.view:GetChild("n26")
end

function BubblePanel:setIndex(index)
    self.childId = index

end

function BubblePanel:setData(data)
    -- printt("聊天气泡>>>>>>>",data)
    if data then
        self.data = data
        self.powerTxt.text = data.power
        self.confData = conf.RoleConf:getChatPaoPao(data)
...
function BubblePanel:titleCelldata(index,obj)
...
function BubblePanel:onClickHaloItem(context)
    local sender = context.sender
    local data = sender.data
    self.chooseData = data
    self.bubbleIcon.url = UIPackage.GetItemURL("_others" , data.icon)
...
--属性加成
function BubblePanel:setAttrsList(data)
    if data and data.id then
        ...
        if data.star_pre then
            local suitStarConf = conf.RoleConf:getSkinsStarAttrData(data.id,1015)
            self.starAttrsList.itemRenderer = function (index,obj)
                ...
                if starData then
                    ...
                    local str2 = string.format(language.fashion15_1,language.gonggong94[1015],(starData.attr_show/100))
...
function BubblePanel:onClickUpStars()
    if self.chooseData then
        if self.chooseData.isHas == 0 then
            GComAlter(language.fashion25)
            return
        end
...
function BubblePanel:onClickWear()
    if self.chooseData then
        if self.chooseData.isHas == 0 then
            --升星消耗是否满足
            ...
            if amount > 0 then
                proxy.PlayerProxy:send(1020506,{reqType = 1,skinId = self.chooseData.id})
            else
                GComAlter(language.fashion23)
                return
            end
        elseif self.chooseData.wear == 0 then--穿戴
            proxy.PlayerProxy:send(1020506,{reqType = 2,skinId = self.chooseData.id})
        elseif self.chooseData.wear == 1 then--卸下
            proxy.PlayerProxy:send(1020506,{reqType = 3,skinId = self.chooseData.id})
        end
    end
end
