...
--请求类型（0.显示 1.升级）
function KageeView:setReqType(reqType)
    if reqType == 1 then
...
--影卫羁绊等级
function KageeView:setYwJbLevel(ywJbLevel)
    self.ywJbLevel = ywJbLevel
end

function KageeView:setData(data)
    self.mData = data
...
function KageeView:cellBtnData(index,cell)
...
--刷新红点
function KageeView:isRefreshRed(cell,data)
...
function KageeView:setFirstData()
...
function KageeView:onClickItem(context)
...
function KageeView:setKageeData(data)
    self.mId = data.id
...
function KageeView:setDownData()
    self.starlev1 = 0
    ...
    self.titleIcon.url = UIPackage.GetItemURL("kagee" , ""..confData.font)
...
--右方属性
function KageeView:setAttiData()
    self:setAllAttiData()
    self:setSelfData()
end
--影卫总属性
function KageeView:setAllAttiData()
...
--本肖属性
function KageeView:setSelfData()
...
function KageeView:refreshLsRed()
    local isRed = false
...
--满级状态
function KageeView:setMaxData(isNotMax)
...
function KageeView:getAngleByPos(p1,p2)  
    local x = p2.x - p1.x  
    local y = p2.y - p1.y
...
--设置形象的星星数据
function KageeView:setStarData()
...
function KageeView:releaseEffect()
...
function KageeView:playEffect()
    ...
    if index > 0 then
        ...
        if self.reqType == 1 then--升星成功播放一个特效
            self.effect1 = self:addEffect(effectId1, effectPanel1)
        end
...
--点击图形的星星
function KageeView:onClickStar(context)
...
function KageeView:onTouchBegin(context)
    local evt = context.data
    self.evtX = evt.x
    self.evtY = evt.y
end

function KageeView:getPos()
    return {x = self.evtX,y = self.evtY}
end
--当前生肖当前阶的星级选择
function KageeView:onClickChoose(context)
...
function KageeView:onClickClose()
    self:closeView()
end

--规则
function KageeView:onClickRule()
    GOpenRuleView(1034)
end

function KageeView:dispose(clear)
    if self.kageeImgPath then
        UnityResMgr:UnloadAssetBundle(self.kageeImgPath, true)
