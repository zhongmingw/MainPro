--
-- Author: 
-- Date: 2018-11-05 22:35:02
--

local QMDJ1002 = class("QMDJ1002")

function QMDJ1002:ctor(mParent)
    self.mParent = mParent
    self:initPanel()

end

function QMDJ1002:initPanel()
...
function QMDJ1002:setData(data)
    self.data = data
    printt("圣装寻宝返还",data)
    self.confData = conf.ActivityConf:getShengZhuangReturnAward()
    self.isGot = {}
    for k,v in pairs(self.data.itemGotSigns) do
        self.isGot[v] = 1
    end
...
function QMDJ1002:getAwards(context)
    local data = context.sender.data
    if data.state == 0 then--不能领
        GComAlter(language.jianLingBorn05)
        return
    else
        proxy.ActivityProxy:sendMsg(1030648,{reqType = 1,cid = data.id})
    end
end

function QMDJ1002:onTimer()
