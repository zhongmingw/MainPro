...
--登录有礼
local Cj1002 = class("Cj1002",import("game.base.Ref"))

function Cj1002:ctor(parent,id)
    self.moduleId = id 
    self.parent = parent
...
function Cj1002:addMsgCallBack(data)
    printt("登录1",data)
    -- body
    self.curDay = data.curDay
    self.normallId = 10000+self.curDay
    local normallData = conf.ChunJieConf2019:getLoginAwardById1(self.normallId)
    if normallData then
        self.normallAwards = normallData.items
        self.listView1.numItems = #self.normallAwards
    end
    self.vipId = 20000+self.curDay
    local vipData = conf.ChunJieConf2019:getLoginAwardById1(self.vipId)
    if vipData then
    ...
    if data.vipAwardSign == 0 then--未领取
        ...
        local needVipLv = conf.ChunJieConf2019:getValue("cj_login_one_vip_level")
...
function Cj1002:onClickGet(context)
    ...
    proxy.ChunJieProxy2019:sendMsg(1030710,{reqType = reqType})
end

function Cj1002:initView()
    -- body
...
function Cj1002:cell1data( index,obj )
...
function Cj1002:cell2data( index,obj )
